...
def generar_reporte(comando):
    ...
    # Futuro: Aquí añadiremos la consulta de gastos.
                    ...
                    if mensaje.get('type') == 'text':
                        ...
                        if negocio.estado_conversacion:
                            ...
                            if estado == 'esperando_nombre_negocio':
                                negocio.nombre = texto_mensaje
                                negocio.estado_conversacion = 'esperando_moneda'
                                db.session.commit()
                                mensaje = f"¡Perfecto! Negocio '{texto_mensaje}' registrado. Ahora, dime la moneda que usas más a menudo (ej. USD o VES)."
                                ...
                                if moneda in ['USD', 'VES']:
                                    negocio.moneda_predeterminada = moneda
                                    negocio.estado_conversacion = 'esperando_primer_producto'
                                    db.session.commit()
                                    mensaje = "👍 Moneda guardada. ¡Ya casi terminamos! Vamos a añadir tu primer producto. ¿Cómo se llama un producto que vendes?"
                                    enviar_a_n8n(numero_usuario, 'texto', {'mensaje': mensaje})
                                else:
                                ...
                                db.session.commit()
                                mensaje = f"Entendido. ¿Y cuántas unidades de '{texto_mensaje}' tienes en stock ahora mismo? (Solo el número)."
                                enviar_a_n8n(numero_usuario, 'texto', {'mensaje': mensaje})

                            elif estado.startswith('esperando_stock_de_'):
                                try:
                                    stock = int(texto_mensaje)
                                    nombre_producto = estado.replace('esperando_stock_de_', '')
                                    ...
                                    mensaje = f"✅ ¡Genial! He añadido '{nombre_producto}' a tu inventario con {stock} unidades.\n\n¡Todo listo! Ya puedes empezar a registrar ventas con `vendí 2 {nombre_producto} por 10`."
                                    enviar_a_n8n(numero_usuario, 'texto', {'mensaje': mensaje})
                                except ValueError:
                        ...
                        if intencion_configurar:
                            ...
                            if ultima_venta:
                                ...
                                db.session.delete(ultima_venta); db.session.commit()
